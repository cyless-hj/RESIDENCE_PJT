...
class GymTransformer:
    ...
    @classmethod
    def transform(cls):
        gym_json = cls._load_json(1)
        data_len = gym_json.select('LOCALDATA_104201.list_total_count').first()[0]
        page_len = data_len // 1000 + 1

        df_gym = cls._select_columns(gym_json)
        df_gym = cls._add_gu_dong_cate_day(df_gym)
        df_gym = cls._add_lat_lon(df_gym)
        df_gym = cls._add_loc_idx(df_gym)
        df_gym = cls._refact_df(df_gym)

        tmp_df = df_gym

        for i in range(2, page_len + 1):
            gym_json = cls._load_json(i)
            df_gym = cls._select_columns(gym_json)
            df_gym = cls._add_gu_dong_cate_day(df_gym)
            df_gym = cls._add_lat_lon(df_gym)
            df_gym = cls._add_loc_idx(df_gym)
            ...
            tmp_df = tmp_df.union(df_gym)

        save_data(DataWarehouse, tmp_df, 'GYM')
    
    @classmethod
    def _refact_df(cls, df):
        df = df.withColumnRenamed('BPLCNM', 'GYM_NAME') \
    ...
    def _add_gu_dong_cate_day(cls, df):
        ...
        cate_day = Row(CATE_CODE='F125', STD_DAY=std_day())
        rows = []
    ...
    def _select_columns(cls, cd_json):
        tmp = cd_json.select('LOCALDATA_104201.row').first()[0]
    ...
    def _load_json(cls, i):
        cd_json = spark_session().read.format("json").json("s3a://residencebucket/raw_data/GYM/GYM_" + std_day() + "_" + str(i) + ".json")
        return cd_json
