...
class SubwayTransformer:
    ...
    def transform(cls):
        ...
        df_sub = cls._add_addr_gu_dong(df_sub)
        df_sub = cls._add_cate_day(df_sub)
        df_sub = cls._add_loc_idx(df_sub)

        df_sub_line = df_sub.select('SUBWAY_NAME', 'SUBWAY_LINE', 'SUBWAY_CODE', 'STD_DAY')
        df_sub = df_sub.select('SUBWAY_NAME', 'ADD_STR', 'SUBWAY_CODE', 'STD_DAY', 'LAT', 'LON', 'CATE_CODE', 'LOC_IDX')
