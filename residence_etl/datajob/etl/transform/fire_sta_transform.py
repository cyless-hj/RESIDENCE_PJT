...
class FireTransformer:
    @classmethod
    def transform(cls):
        df_fire = cls._load_csv()
        df_fire = cls._add_addr_gu_dong(df_fire)
        df_fire = cls._add_cate_day(df_fire)
        df_fire = cls._add_loc_idx(df_fire)

        save_data(DataWarehouse, df_fire, 'FIRE_STA')
    ...
    def _load_csv(cls):
        df = spark_session().read.format("csv") \
                                 ...
                                 .csv("s3a://residencebucket/raw_data/FIRE_STA/FIRE_STA.csv")
    ...
    def _add_cate_day(cls, df):
        ...
        cate_day = Row(CATE_CODE='B112', STD_DAY=std_day())
        rows = []
