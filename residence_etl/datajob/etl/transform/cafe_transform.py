...
class CafeTransformer:
    @classmethod
    def transform(cls):
        df_cafe = cls._load_csv()
        df_cafe = cls._add_gu_dong_cate_day(df_cafe)
        df_cafe = cls._add_loc_idx(df_cafe)
        df_cafe = cls._add_lat_lon(df_cafe)

        save_data(DataWarehouse, df_cafe, 'CAFE')
    ...
    def _add_gu_dong_cate_day(cls, df_cafe):
        ...
        cate_day = Row(CATE_CODE='C112', STD_DAY=std_day())
        rows = []
    ...
    @classmethod
    def _load_csv(cls):
        df_cafe = spark_session().read.format("csv") \
                                 .option("header", "true") \
