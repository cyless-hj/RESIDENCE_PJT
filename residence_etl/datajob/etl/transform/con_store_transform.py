...
class ConstoreTransformer:
    @classmethod
    def transform(cls):
        df_con = cls._load_csv()
        df_con = cls._add_gu_dong_cate_day(df_con)
        df_con = cls._add_loc_idx(df_con)
        df_con = cls._add_lat_lon(df_con)

        save_data(DataWarehouse, df_con, 'CON_STORE')
    ...
    def _add_gu_dong_cate_day(cls, df_cafe):
        ...
        cate_day = Row(CATE_CODE='C113', STD_DAY=std_day())
        rows = []
