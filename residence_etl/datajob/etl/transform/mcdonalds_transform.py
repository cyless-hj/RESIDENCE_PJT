...
class McdonaldsTransformer:
    @classmethod
    def transform(cls):
        df_mc = cls._load_csv()
        df_mc = cls._add_gu_dong_cate_day(df_mc)
        df_mc = cls._add_loc_idx(df_mc)
        df_mc = cls._add_lat_lon(df_mc)

        save_data(DataWarehouse, df_mc, 'MCDONALDS')
    ...
    def _load_csv(cls):
        df = spark_session().read.format("csv") \
                                 ...
                                 .csv("s3a://residencebucket/raw_data/MCDONALDS/MCDONALDS.csv")
