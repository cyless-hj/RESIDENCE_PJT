...
class CctvTransformer:
    @classmethod
    def transform(cls):
        df_cctv = cls._load_csv()
        df_cctv = cls._add_addr_gu_dong(df_cctv)
        df_cctv = cls._add_cate_day(df_cctv)
        df_cctv = cls._add_loc_idx(df_cctv)

        save_data(DataWarehouse, df_cctv, 'CCTV')
    ...
    def _load_csv(cls):
        df = spark_session().read.format("csv") \
                                 ...
                                 .csv("s3a://residencebucket/raw_data/CCTV/CCTV.csv")
    ...
    def _add_cate_day(cls, df):
        ...
        cate_day = Row(CATE_CODE='B114', STD_DAY=std_day())
        rows = []
